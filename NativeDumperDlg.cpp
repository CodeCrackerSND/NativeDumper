// NativeDumperDlg.cpp : implementation file
//
...
CNativeDumperDlg::CNativeDumperDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CNativeDumperDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CNativeDumperDlg)
...
void CNativeDumperDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CNativeDumperDlg)
	DDX_Control(pDX, IDC_LIST1, m_cListCtrl);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CNativeDumperDlg, CDialog)
	//{{AFX_MSG_MAP(CNativeDumperDlg)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
/////////////////////////////////////////////////////////////////////////////
// CNativeDumperDlg message handlers

BOOL CNativeDumperDlg::OnInitDialog()
{
...
HCURSOR CNativeDumperDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

BOOL CNativeDumperDlg::EnableDebugPrivileges()
{
    HANDLE tokenHandle;
    LUID luid;
...
void CNativeDumperDlg::RefreshProcessList()
{
...
void CNativeDumperDlg::OnRclickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
CMenu menu;
menu.LoadMenu(IDR_PROCESSMENU); // our context menu
...
BOOL CNativeDumperDlg::OnCommand(WPARAM wParam, LPARAM lParam) 
{
	...
	if (HIWORD(wParam) == BN_CLICKED)  // if button clicked
    {
		...
		modulesdlg.processname = m_cListCtrl.GetItemText(position, 0);  // item number, subitem number
		CString pid_str = m_cListCtrl.GetItemText(position, 1);  // 1 since we need process id
		modulesdlg.processid = GenericPurposeMethods::StringToNumber(pid_str.GetBuffer(pid_str.GetLength()));
		//testdlg.DoModal();  // not modal one!
		BOOL ret = modulesdlg.Create(IDD_MODULES,this);
		...
		case ID_DUMPMAINMODULE:
		{

		if (!IsWindow(dmoduledlg.m_hWnd)||!dmoduledlg.IsWindowVisible())
		{

		DumpModuleDlg m_pdmoduledialog = new DumpModuleDlg(this);
		dmoduledlg = m_pdmoduledialog;
		POSITION pos = m_cListCtrl.GetFirstSelectedItemPosition();
		int position = m_cListCtrl.GetNextSelectedItem(pos);
		
		dmoduledlg.processname = m_cListCtrl.GetItemText(position, 0);  // item number, subitem number
		CString pid_str = m_cListCtrl.GetItemText(position, 1);  // 1 since we need process id
		dmoduledlg.processid = GenericPurposeMethods::StringToNumber(pid_str.GetBuffer(pid_str.GetLength()));

	// Get a handle to the process.
    HANDLE hProcess = OpenProcess( PROCESS_QUERY_INFORMATION |
                                   PROCESS_VM_READ,
                                   FALSE, dmoduledlg.processid );
    ...
    if (NULL != hProcess )
    {
		...
		dmoduledlg.shortmodulename[i]=szProcessName[i];  // copy short name
		...
		dmoduledlg.fullmodulename[j]=szProcessName[j];  // copy full name

		dmoduledlg.hMod = hMod;  // set module address!

		//testdlg.DoModal();  // not modal one!
		BOOL ret = dmoduledlg.Create(IDD_DUMPMODULEDLG_DIALOG,this);
		if (ret)   // If create not failed.
		dmoduledlg.ShowWindow(SW_SHOWNORMAL);

        }
    }
		}

		}  // end of window visible!
